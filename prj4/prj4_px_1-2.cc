...
bool rtVirtualSend (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
    ...
    // We can decide the router for every packet                                //
    {
        MyTag mytag;
        if (counter == 2) {
            mytag.SetSimpleValue (3);                                 // add packet sequence number at the router
        } else if (counter == 3) {
            mytag.SetSimpleValue (2);                                 // add packet sequence number at the router
        } else {
            mytag.SetSimpleValue (counter);                                 // add packet sequence number at the router
        }
        packet->AddByteTag(mytag);  
        ...
        if (Scenario == AGGREGATE) {
            ...
            if (aggPath.compare("lteOnly") == 0) {
                ...
                double wifiScore = (wifiDelay < lteDelay) && (wifiThroughput > lteThroughput) ? 3 : (wifiDelay < lteDelay) ? 2 : (wifiThroughput > lteThroughput) ? 1 : 0;
                double lteScore = (lteDelay < wifiDelay) && (lteThroughput > wifiThroughput) ? 3 : (lteDelay < wifiDelay) ? 2 : (lteThroughput > wifiThroughput) ? 1 : 0;

                if (lteScore > wifiScore) {
                    // Use LTE path for DL traffic
                    m_rtSocket->SendTo (packet, 0, InetSocketAddress (m_msIfc0Address, TunnelPort)); 
                    lteDataSent += packet->GetSize();

                } else if (wifiScore > lteScore) {
                    // use Wi-Fi path DL traffic
                    m_rtSocket->SendTo (packet, 0, InetSocketAddress (m_msIfc1Address, TunnelPort));
                    wifiDataSent += packet->GetSize();
